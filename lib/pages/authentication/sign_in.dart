...
import 'package:chanzo/pages/authentication/kiotapay_reset_password_email.dart';
...
// Load the saved value
  ...
  Future<void> loginWithEmail() async {
    ...
    try {
      ...
      // Update FCM token
      await FCMTokenManager.handleToken(null);

      // Apply context payload safely
      await authController.applyLoginPayload(data);

      // Determine if the user has multiple branches or multiple roles in a single branch
      final availableContexts = authController.availableContexts;
      final hasMultipleAccounts = availableContexts.length > 1 ||
          (availableContexts.isNotEmpty && availableContexts.first.roles.length > 1);

      // If backend mandates it OR if the user actually has multiple accounts, force the switcher
      final action = (data['context_action'] ?? 'skip').toString();
      final requiresContext = data['requires_context_selection'] == true;

      if (requiresContext || action == 'select_branch' || action == 'select_role' || hasMultipleAccounts) {
        hideLoading();
