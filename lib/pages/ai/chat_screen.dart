...
class _ChatPageScreenState extends State<ChatPageScreen> {
  ...
  String thinkingStatus = "Thinking...";
  ...
  void fetchSessions({bool reset = false}) async {
    ...
    final uri = Uri.parse(
            "${KiotaPayConstants.fetchSessions}")
  ...
  void loadMessagesFromSession(String sessionId) async {
    ...
    final uri = Uri.parse("${KiotaPayConstants.loadMessagesFromSession}/$sessionId/history");
    final client = HttpClient();
    final request = await client.getUrl(uri);

    // SET HEADERS
    request.headers.set('Authorization', 'Bearer ${authController.token}');
  ...
  void aiStream(String input) async {
    ...
    setState(() {
      ...
      isThinking = true;
      thinkingStatus = "Thinking..."; // Reset status on new prompt
      showWelcome = false;
    });
    _scrollToBottom();
    _controller.clear();
    ...
    //   currentSessionId = '${DateTime.now().millisecondsSinceEpoch}_${authController.userId}';
    // }
    ...
    final request = {
      ...
      if (currentSessionId.isNotEmpty) "thread_id": currentSessionId,
    };

    try {
      final client = HttpClient();
      final uri = Uri.parse(KiotaPayConstants.aiStream);
      final requestBody = utf8.encode(jsonEncode(request));
      print("String URL is $uri");

      final req = await client.postUrl(uri);
      req.headers.set('Authorization', 'Bearer ${authController.token}');
      ...
      String currentEvent = ""; // Track the current SSE event type
          ...
          for (String line in lines) {
            ...
            if (line.startsWith("data:")) {
              try {
                ...
                if (currentEvent == "status" && jsonLine["message"] != null) {
                  setState(() {
                    thinkingStatus = jsonLine["message"];
                  });
                  continue;
                }

                // Intercept Official Thread ID
                if (currentEvent == "thread_id" && jsonLine["thread_id"] != null) {
                  currentSessionId = jsonLine["thread_id"];
                  continue;
                }

                // Intercept Chat Stream
                if (currentEvent == "on_chat_model_stream" && jsonLine["node"] == "agent" && jsonLine["data"] != null) {
                  streamedText += jsonLine["data"];
                  setState(() {
                    if (isFirstChunk) {
                      isThinking = false; // Hide typing indicator once text arrives
                      messages.add({
                        "type": "ai",
                ...
                // Intercept Errors
                if (currentEvent == "error" && jsonLine["error"] != null) {
                  setState(() {
                    isThinking = false;
                    messages.add({
                      "type": "ai",
                      "content": "❌ ${jsonLine['error']}",
                      "isMarkdown": false,
                      "timestamp": DateTime.now().toIso8601String(),
                    });
                    _scrollToBottom();
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: [
            ...
            Flexible(
              child: Text(
                thinkingStatus, // eg "Checking school fee balances..."
                style: TextStyle(
  ...
  Widget _buildPromptButton(String label) {
    return ElevatedButton(
      ...
      onPressed: () {
        aiStream(label); // directly send on click
      },
      child: Text(label),
    );
  }
