...
class FCMTokenManager {
  ...
  static Future<void> _sendTokenToBackend({
    ...
    // If user is not logged in, just abort quietly. It will send when they log in.
    if (token == null) return;
    ...
    try {
      ...
      if (response.statusCode != 200) {
        // Just print it so it doesn't crash the app!
        print('Backend rejected token. Status: ${response.statusCode}');
        Get.snackbar('FCM Initialize failed', 'Backend rejected token. Status: ${response.statusCode}', backgroundColor: Colors.orange, colorText: Colors.white);
      }
    } catch (e) {
      Get.snackbar('FCM Initialize failed', 'Token submission failed (Network/Timeout)', backgroundColor: Colors.orange, colorText: Colors.white);
      // Catch all network errors quietly
      print('Token submission failed (Network/Timeout): $e');
    }
  }
