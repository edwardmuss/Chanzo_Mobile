...
class _KiotaPaySettingsState extends State<KiotaPaySettings> {
  ...
  Future<void> verifyPinBiometric(
    ...
    var body = {"user_pin": pinCode};
    showLoading("Verifying PIN");
    try {
      var url = Uri.parse(KiotaPayConstants.verifyUserPin);
      ...
      if (response.statusCode == 200) {
        ...
        if (error_message == "Pin mismatch!") {
          awesomeDialog(context, "Failed", "You have entered a wrong PIN", true,
              DialogType.error, ChanzoColors.secondary)
            ..show();
          setState(() {
            // pinError = "You have entered a wrong PIN";
          });
        } else {
          awesomeDialog(context, "Failed", error_message, true,
              DialogType.error, ChanzoColors.secondary)
            ..show();
        }
      }
  ...
  Future<void> resetPassword() async {
    ...
    var body = {
      "previous_pin": currentPinController.text,
      "new_pin": newPinController.text,
      "confirm_pin": confirmNewController.text,
    };
    showLoading("Just a moment");
    try {
      var url = Uri.parse(KiotaPayConstants.resetUserPin);
      http.Response response =
      ...
      if (response.statusCode == 200) {
        ...
        awesomeDialog(context, "Success", json['msg'], true, DialogType.info,
            ChanzoColors.primary)
          ..show();
      } else {
        hideLoading();
        var error_message = jsonDecode(response.body)['message'];
        awesomeDialog(context, "Failed", error_message, true, DialogType.error,
            ChanzoColors.secondary)
          ..show();
        setState(() {
          // pinError = "You have entered a wrong PIN";
